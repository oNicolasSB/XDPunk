...
cleanup() {
  ...
  rm -f /tmp/xdp_forward.c /tmp/xdp_forward.o
}
trap cleanup EXIT
...
echo "[4/7] Gerando programa eBPF (XDP)..."

cat > /tmp/xdp_forward.c <<'EOF'
#include <linux/bpf.h>
...
int xdp_forward(struct xdp_md *ctx)
{
    ...
    if (eth->h_proto == bpf_htons(ETH_P_ARP)) {
        if (ctx->ingress_ifindex == IF1) return bpf_redirect(IF2, 0);
        if (ctx->ingress_ifindex == IF2) return bpf_redirect(IF1, 0);
        if (ctx->ingress_ifindex == IF3) return bpf_redirect(IF1, 0);
    }
...
clang -O2 -g -target bpf \
  ...
  -DIF2="$IF2" \
  -DIF3="$IF3" \
  -c /tmp/xdp_forward.c -o /tmp/xdp_forward.o

echo "[7/7] Carregando programas XDP..."

for dev in "$VETH1S" "$VETH2S" "$VETH3S"; do
  ip netns exec "$NSSW" ip link set dev "$dev" xdpgeneric obj /tmp/xdp_forward.o sec xdp
done
